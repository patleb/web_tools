class TaskJob < ActiveJob::Base
  def perform(name, *arguments)
    ...
    Flash[:error] = I18n.t('task.flash.failure_html', name: task.name, path: task.path) + ExtRails::ERROR_SEPARATOR
    Flash[:error] += (errors = task.errors.full_messages.join(ExtRails::ERROR_SEPARATOR)) # should be only one error
