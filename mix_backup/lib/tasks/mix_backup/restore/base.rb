module MixBackup
  module Restore
    class Base < Base
      ...
      def s3_storage
        @_s3_storage ||= File.join(MixBackup.config.backup_s3_path, directory.tr('-', '.'))
      end

      def local_storage
        @_local_storage ||= begin
          local_storage = MixBackup.config.backup_local_path.join(directory)
      ...
      def bucket
        @_bucket ||= s3.bucket(MixBackup.config.s3_bucket)
      end

      def s3
        @_s3 ||= Aws::S3::Resource.new(
          access_key_id: MixBackup.config.s3_access_key_id,
          secret_access_key: MixBackup.config.s3_secret_access_key,
          region: MixBackup.config.s3_region
        )
      end
    end
  end
