namespace! :page do
  ...
  namespace :images do
    ...
    task :prod_to_dev => :environment do
      prod_url = base_url :production
      dev_url = base_url :development
      convert_fields prod_url, dev_url
    end

    desc 'Replace production base url for staging'
    task :prod_to_staging => :environment do
      prod_url = base_url :production
      staging_url = base_url :staging
      convert_fields prod_url, staging_url
    end
  end
