module MrTemplate
  module ViewHelper
    ...
    def browser_upgrade_html
      "<!--[if lt IE 11]><p class='browser_upgrade'>#{t('mr_template.browser_upgrade_html')}</p><![endif]-->".html_safe
    end

    def js_i18n(key = 'mr_template.js')
    ...
    def back_to_site_link(target_blank = true)
      ...
      if target_blank
        a_ [t('mr_template.back_to_site'), i_('.fa.fa-external-link.sidebar_external')], href: back_to_site_path, target: '_blank'
      else
        link_to t('mr_template.back_to_site'), back_to_site_path
      end
    end
