...
class NetCDFTest < Rice::TestCase
  ...
  test '#open, #write, #read' do
    ...
    assert(NetCDF::File.open(path, 'w') do |f|
      ...
      none = f.create_var :none, :UInt8, [:n, :y, :x]
      ...
      assert_equal 0, null.shape.size
      ...
      f.write_att :null, :val, Tensor::SFloat[12345]
      ...
      assert_equal [12345.0.to_s], f.read_att(:null, :val).map(&:to_s)
