...
job_type :bash_script,  "#{Sh.rbenv_ruby} #{context} #{path} #{mutex} '#{flock} nice -n 19 #{bash}/:task.sh :output'"
job_type :bash_script!, "#{Sh.rbenv_ruby} #{context} #{path} #{mutex} '#{flock} #{bash}/:task.sh :output'"
job_type :bash_helper,  "#{Sh.rbenv_ruby} #{context} #{path} #{mutex} '#{helper} #{flock} nice -n 19 #{bash}/helper.sh :output #{no_helper}'"
job_type :bash_helper!, "#{Sh.rbenv_ruby} #{context} #{path} #{mutex} '#{helper} #{flock} #{bash}/helper.sh :output #{no_helper}'"
