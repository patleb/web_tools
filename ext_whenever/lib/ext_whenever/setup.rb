...
rake       = "#{rbenv_ruby}; #{rbenv_sudo} bin/rake"
runner     = "#{rbenv_ruby}; #{rbenv_sudo} bin/rails runner"

job_type :rake,    "#{path} #{flock} nice -n 19 #{rake} :task --silent :output"
job_type :rake!,   "#{path} #{flock} #{rake} :task --silent :output"
job_type :runner,  "#{path} #{flock} nice -n 19 #{runner} ':task' :output"
job_type :runner!, "#{path} #{flock} #{runner} ':task' :output"
